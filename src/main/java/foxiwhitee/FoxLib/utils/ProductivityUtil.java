...
public class ProductivityUtil {
    ...
    public static double gaugeProductivityProgressBar(double ticks, int percent, double[] progress, double pix) {
        double n = findNumberOfCycles(percent);

        double x = n != 0 ? progress[0] / n + (ticks / (20 * FoxConfig.cobblestoneDuperSecondsNeed)) / n : 0;
